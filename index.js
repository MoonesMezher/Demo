...
const socialLinksData = [
    { name: 'facebook', href: '#', svg: '<path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>' },
    { name: 'twitter', href: '#', svg: '<path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231 5.451-6.231zm-1.161 17.52h1.833L7.084 4.126H5.117L17.083 19.77z" fill="currentColor"/>' },
    ...
    { name: 'bihance', href: '#', svg: '<img width="20" height="20" src="https://img.icons8.com/windows/32/behance.png" alt="behance"/>' }
];
    ...
    sections: [
        {
            ...
            features: [
                ...
                { name: "Bandwidth", values: ["100 G.B", "200 G.B", "غير محدود"] },
                { name: "النطاقات الفرعية", values: ["غير محدود", "غير محدود", "غير محدود"] },
                { name: "قواعد البيانات", values: ["غير محدود", "غير محدود", "غير محدود"] },
                { name: "حسابات FTP", values: ["غير محدود", "غير محدود", "غير محدود"] },
                { name: "البريد الإلكتروني", values: ["غير محدود", "غير محدود", "غير محدود"] }
            ]
        },
        {
            title: "ميزات الإيميل",
            ...
            features: [
                { name: "سعة البريد", values: ["2 GB", "5 GB", "10 GB"] },
                { name: "تصفية البريد العشوائي", values: ["✓", "✓", "✓"] },
                { name: "دعم IMAP/POP3", values: ["✓", "✓", "✓"] },
                { name: "البريد الوارد", values: ["غير محدود", "غير محدود", "غير محدود"] }
            ]
        },
        {
            ...
            features: [
                { name: "MySQL Databases", values: ["غير محدود", "غير محدود", "غير محدود"] },
                { name: "phpMyAdmin", values: ["✓", "✓", "✓"] },
                { name: "نسخ احتياطي", values: ["يومي", "يومي", "يومي"] }
            ]
        },
        {
            title: "خيارات الدفع",
            ...
            features: [
                { name: "البطاقات الائتمانية", values: ["✓", "✓", "✓"] },
                { name: "PayPal", values: ["✓", "✓", "✓"] },
                { name: "التحويل البنكي", values: ["✓", "✓", "✓"] },
                { name: "الدفع السنوي", values: ["خصم 10%", "خصم 15%", "خصم 20%"] }
            ]
        }
    ]
};
...
function createComparisonTable() {
        ...
        planHeaderCell.innerHTML = `
            <div class="bg-white scale-75 xs:scale-90 sm:scale-100 text-black p-4 xs:p-6 sm:p-8 text-center">
                ...
                <div class="mt-2 xs:mt-4 text-dark">
                    <span class="text-lg xs:text-xl">شهرياً</span>
                    <span class="text-4xl xs:text-5xl sm:text-6xl font-black">
                        <span class="font-normal">/</span>${plan.price}
                        <span class="text-base xs:text-lg align-middle">$</span>
                    </span>
                </div>
    ...
    comparisonTableData.sections.forEach((section, i) => {
        ...
        sectionHeaderRow.innerHTML = `
            <td colspan="4">
                <div class="p-3 xs:p-4 font-bold w-full flex justify-between items-center text-dark text-lg xs:text-xl bg-[#d1d5db] border border-solid border-transparent border-b-[#939598]">
                    <span class="min-w-[260px] text-center">${section.title}</span>
        ...
        section.features.forEach((feature) => {
            ...
            featureNameCell.className = 'font-bold p-3 xs:p-4 bg-gray-50 min-w-[240px]';
            ...
            feature.values.forEach((value) => {
                ...
                valueCell.className = `p-3 xs:p-4 text-[#282828]`;
...
function createNavigation() {
    ...
    navigationData.forEach((item, i) => {
        ...
        mobileLink.className = 'hover:text-primary';
    ...
    plansData.forEach(plan => {
        ...
        planElement.innerHTML = `
            <div class="bg-${plan.color} rounded-2xl overflow-hidden fade-in">
                ...
                <div class="cursor-pointer w-full h-[60px] bg-${plan.color} text-center flex justify-center items-center text-[24px] duration-300 hover:h-[70px]">
